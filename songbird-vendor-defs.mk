...
SB_USE_MINGW_GCC := $(SB_USE_MINGW_GCC)
...
ifeq (Msys,$(SB_VENDOR_OS))
  ifneq (1, $(SB_USE_MINGW_GCC))
    ...
    SB_TARGET_ARCH := windows-i686-mingw32
    SB_ARCH_DETECTED := 1
    SB_VENDOR_ARCH := Mingw
  endif
endif
...
ifneq (,$(call enable-sb-lib, libtool))
  ifneq ($(wildcard $(SB_VENDOR_BINARIES_DIR)/libtool/release), )
    ...
    SB_ACLOCAL_FLAGS += -I $(SB_LIBTOOL_DIR)/share/aclocal
  endif
endif
...
ifeq (Msys,$(SB_VENDOR_ARCH))
  ifneq (,$(call enable-sb-lib, zlib))
    ifneq ($(wildcard $(SB_VENDOR_BINARIES_DIR)/zlib/$(SB_BUILD_TYPE)), )
      ...
      SB_LDFLAGS += -L$(SB_ZLIB_DIR)/lib -lzlib
      SB_CFLAGS += -I$(SB_ZLIB_DIR)/include
