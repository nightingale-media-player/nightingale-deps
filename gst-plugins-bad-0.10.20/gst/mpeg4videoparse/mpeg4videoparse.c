...
gst_mpeg4vparse_sink_setcaps (GstPad * pad, GstCaps * caps)
{
  ...
  if ((value = gst_structure_get_value (s, "codec_data")) != NULL
    ...
    if (!parse->config) {
      parse->config = gst_buffer_new_and_alloc (GST_BUFFER_SIZE (buf));
      memcpy (GST_BUFFER_DATA (parse->config), GST_BUFFER_DATA (buf),
              GST_BUFFER_SIZE (buf));
    }
...
failed_parse:
  /* No codec data, or obviously-invalid, so set minimal new caps.. 
     VOS parsing later will fill in * the other fields */
