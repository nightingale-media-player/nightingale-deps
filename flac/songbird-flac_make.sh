...
build()
{
	...
	export CFLAGS="${CFLAGS} -D_MSC_VER=${_MSC_VER} -DWIN32 -D__NO_CTYPE -D_CRT_SECURE_NO_DEPRECATE  -DHAVE_WIN32 -D_WINDOWS -wd4820 -wd4668 -wd4100 -wd4706 -wd4127 -wd4255 -wd4710 -wd4055 -DFLAC_API_EXPORTS"
    ...
    if [ "$sys_name" = "Darwin" ]; then
        install_name_tool                                                      \
            -id libFLAC.dylib                                                \
            ${dep_arch_dir}/${tgt_name}/${build_type}/lib/libFLAC.dylib
    fi
