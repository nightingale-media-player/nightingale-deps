...
build()
{
	...
	cfg_opts="${cfg_opts} --with-ogg-libraries=${tgt_dep_dir}/lib"
	cfg_opts="${cfg_opts} --with-ogg-includes=${tgt_dep_dir}/include"
        if [ "$sys_name" = "Darwin" ]; then
            export_append                                                      \
                        "LDFLAGS"                                              \
                        "-Wl,-dylib_file"                                          \
                        "-Wl,libogg.dylib:${tgt_dep_dir}/lib/libogg.dylib"
            export DYLD_LIBRARY_PATH="${tgt_dep_dir}/lib:${DYLD_LIBRARY_PATH}"
	elif [ "$sys_name" = "Cygwin" ]; then
    ...
    if [ "$sys_name" = "Darwin" ]; then
        install_name_tool                                                      \
            -id libvorbis.dylib                                                \
            ${dep_arch_dir}/${tgt_name}/${build_type}/lib/libvorbis.dylib

        install_name_tool                                                      \
            -id libvorbisenc.dylib                                             \
            ${dep_arch_dir}/${tgt_name}/${build_type}/lib/libvorbisenc.dylib
        install_name_tool                                                      \
            -change                                                            \
              ${dep_arch_dir}/${tgt_name}/${build_type}/lib/libvorbis.0.dylib  \
              libvorbis.dylib                                                  \
            ${dep_arch_dir}/${tgt_name}/${build_type}/lib/libvorbisenc.dylib

        install_name_tool                                                      \
            -id libvorbisfile.dylib                                            \
            ${dep_arch_dir}/${tgt_name}/${build_type}/lib/libvorbisfile.dylib
        install_name_tool                                                      \
            -change                                                            \
              ${dep_arch_dir}/${tgt_name}/${build_type}/lib/libvorbis.0.dylib  \
              libvorbis.dylib                                                  \
