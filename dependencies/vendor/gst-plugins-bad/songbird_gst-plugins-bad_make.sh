...
################################################################################
#
# Script for building gst-plugins-bad
#
################################################################################
################################################################################
...
build()
{
    ...
    export GST_PLUGINS_BASE_CFLAGS=
    export GST_PLUGINS_BASE_LIBS=
	...
	export PKG_CONFIG_PATH="${PKG_CONFIG_PATH}:${tgt_dep_dir}/lib/pkgconfig"
        ...
        export_append "GST_PLUGINS_BASE_LIBS"                                           \
                      "-L${tgt_dep_dir}/lib ${GST_LIBS}"                                   \
                      "-lgstreamer-0.10 ${GLIB_LIBS}"
        export_append "GST_PLUGINS_BASE_CFLAGS"                                         \
                      "-I${tgt_dep_dir}/include/gstreamer-0.10 ${GST_CFLAGS}"
	export GSTPB_TOOLS_DIR="${tgt_dep_dir}/bin"
	export GSTPB_PLUGINS_DIR="${tgt_dep_dir}/lib/gstreamer-0.10"
	export PKG_CONFIG_PATH="${PKG_CONFIG_PATH}:${tgt_dep_dir}/lib/pkgconfig"
        if [ "$sys_name" = "Darwin" ]; then
            export_append                                                      \
                        "LD_FLAGS"                                              \
	    ...
	    export PATH="$PATH:${tgt_dep_dir}/bin"
	    if [ "$build_type" = "debug" ]; then
		export GST_PLUGINS_BASE_LIBS="$GST_PLUGINS_BASE_LIBS -Wl,-Zi"
	    fi
        fi
    ...
    if test -n "${cl_process}"; then
        ...
        export GST_PLUGINS_BASE_CFLAGS=`${cl_process} echo ${GST_PLUGINS_BASE_CFLAGS}`
        export GST_PLUGINS_BASE_LIBS=`${cl_process} echo ${GST_PLUGINS_BASE_LIBS}`
    ...
    if [ "$sys_name" = "Darwin" ]; then
        install_name_tool                                                      \
            -id libgstapp-0.10.dylib                                           \
            ${dep_arch_dir}/${tgt_name}/${build_type}/lib/libgstapp-0.10.dylib
        install_name_tool                                                      \
            -change                                                            \
              ${dep_arch_dir}/${tgt_name}/${build_type}/lib/libgstapp-0.10.0.dylib \
              libgstapp-0.10.dylib                                             \
            ${dep_arch_dir}/${tgt_name}/${build_type}/lib/gstreamer-0.10/libgstapp.so
    fi
