...
/* === BEGIN SONGBIRD ADDITIONS === */
int quicktime_dump_atom(quicktime_t *file, quicktime_atom_t *parent_atom, char *buffer, void (*callback)( const char *atom_path, const char *value_string, void *context_data ), void *context)
{
  volatile static int foo;
  int result = 0;
  quicktime_atom_t leaf_atom;

  /* Append the type to the buffer to preserve the hierarchy. */
  int len = strlen( buffer );
  if ( len + 5 > 2048 ) return 0; /* No buffer overwrite, please.   Is this still exploitable? */
  if ( len )
  {
    buffer[ len ] = '.';
    ++len;
  }
  memcpy(buffer + len, parent_atom->type, 4);
  buffer[ len + 4 ] = 0; /* Null term */
  ...
  do
    ...
    /* Read a 32 bit value.   */
    check32 = quicktime_read_int32( file );
    /* If it is a 0, flush the 0?  Yay for a wacky "meta" atom! */
    if ( check32 )
      /* Otherwise, that was important, put it back. */
      quicktime_set_position(file, quicktime_position(file) - 4);  

    /* Make a new name-buffer for each child node. */
    strcpy( out_buffer, buffer );
    result = quicktime_atom_read_header(file, &leaf_atom);
    if ( !result )
    {
      quicktime_dump_atom(file, &leaf_atom, out_buffer, callback, context);
      file->file_position = leaf_atom.end;
    }
...
int quicktime_dump_info(quicktime_t *file, void (*callback)( const char *atom_path, const char *value_string, void *context_data ), void *context)
{
  int result = 0, found_moov = 1;
  ...
  do
  {
    ...
    if ( !result )
    {
      quicktime_dump_atom(file, &leaf_atom, buffer, callback, context);
      file->file_position = leaf_atom.end;
    }
  }while(!result && quicktime_position(file) < file->total_length);
