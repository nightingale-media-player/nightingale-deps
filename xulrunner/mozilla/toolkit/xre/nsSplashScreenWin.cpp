...
#else
#pragma message ("WARNING: splashscreen needs some code to load bitmaps on non-WinCE")
    ...
    if (mSplashBitmap) {
        // XXX Songbird: only show the splash screen if a splash image is available (bug 17105)
	DWORD threadID = 0;
...
#if 0 // XXX Songbird: removed to never show progress bar (bug 17105)
    if (mProgress != -1) {
