             ...
             unsigned int level)
{
    ...
    for(x = 0, y = 0; expr[y]; ++y, ++x) {
        ...
        case '[': {
            ...
            if (start == '\\')
                start = expr[i++];
            if (alphanumeric<T>(start) && expr[i++] == '-') {
                end = expr[i++];
                if (end == '\\')
                    end = expr[i++];
            }
            if (alphanumeric<T>(end) && expr[i] == ']') {
                ...
                if (case_insensitive && alpha<T>(val)) {
                ...
                for (; expr[y] != ']'; y++) {
                    if (expr[y] == '\\')
                        ++y;
                    if(case_insensitive)
                        matched |= (upper<T>(str[x]) == upper<T>(expr[y]));
                    else
        ...
        default:
            if(case_insensitive) {
                if(upper<T>(str[x]) != upper<T>(expr[y]))
                    return NOMATCH;
            }
            else {
                if(str[x] != expr[y])
...
ns_WildCardMatch(const T *str, const T *xp, PRBool case_insensitive)
{
    ...
    if (!nsCharTraits<T>::find(xp, nsCharTraits<T>::length(xp), T('~')))
        return _shexp_match<T>(str, xp, case_insensitive, 0);
...
NS_WildCardMatch_(const T *str, const T *expr, PRBool case_insensitive)
{
    ...
    switch(is_valid) {
        case INVALID_SXP:
            return -1;
        default:
            return ns_WildCardMatch<T>(str, expr, case_insensitive);
    }
}
