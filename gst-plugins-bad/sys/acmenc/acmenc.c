...
BOOL CALLBACK acmenc_format_enum (HACMDRIVERID driverId, LPACMFORMATDETAILS fd, DWORD_PTR dwInstance, DWORD fdwSupport)
{
  ...
  if (!enc->outfmt) {
    ...
    enc->outfmt = (WAVEFORMATEX *)g_malloc (fd->pwfx->cbSize + sizeof(WAVEFORMATEX));
...
gboolean acmenc_set_format (ACMEnc *enc)
{
  ...
  res = acmMetrics ((HACMOBJ)enc->driver, ACM_METRIC_MAX_SIZE_FORMAT, &maxSize);
  ...
  res = acmFormatEnum(enc->driver, &details, acmenc_format_enum, (DWORD_PTR)enc, ACM_FORMATENUMF_CONVERT);
...
acmenc_setup (ACMEnc *enc)
{
  ...
  res = acmStreamOpen(&enc->stream, enc->driver, &enc->infmt, enc->outfmt,
...
static gboolean acmenc_sink_setcaps (GstPad * pad, GstCaps * caps)
{
...
acmenc_push_output (ACMEnc *enc)
{
  ...
  if (enc->header.cbDstLengthUsed > 0) {
    ...
    memcpy (GST_BUFFER_DATA (outbuf), enc->header.pbDst, enc->header.cbDstLengthUsed);
    ...
    if (enc->outfmt->nAvgBytesPerSec > 0) {
      ...
      GST_BUFFER_TIMESTAMP (outbuf) = gst_util_uint64_scale_int (
            enc->bytes_output, GST_SECOND, enc->outfmt->nAvgBytesPerSec);
    }
    enc->bytes_output += enc->header.cbDstLengthUsed;

    GST_DEBUG_OBJECT (enc, "Pushing %d byte encoded buffer", enc->header.cbDstLengthUsed);
...
static GstFlowReturn acmenc_chain (GstPad * pad, GstBuffer * buf)
{
  MMRESULT res;
  ACMEnc *enc = (ACMEnc *)GST_PAD_PARENT (pad);
  ...
  while (len) {
    ...
    res = acmStreamConvert (enc->stream, &enc->header, 
    ...
    if (enc->header.cbSrcLengthUsed > 0)       
    {
      if (enc->header.cbSrcLengthUsed != enc->header.cbSrcLength) {
        ...
        memmove (enc->header.pbSrc, enc->header.pbSrc + enc->header.cbSrcLengthUsed,
...
static gboolean acmenc_sink_event (GstPad * pad, GstEvent * event)
{
  ACMEnc *enc = (ACMEnc *)GST_PAD_PARENT (pad);
  gboolean res;
...
{
  enc->sinkpad = gst_pad_new_from_static_template (&acmenc_sink_template, "sink");
...
acmenc_class_init (ACMEncClass *klass)
{
  ...
  g_object_class_install_property (gobjectclass, ARG_BITRATE,
      g_param_spec_int ("bitrate", "Bitrate",
          "Bitrate to encode at (in bps)", 0, 1000000, DEFAULT_BITRATE,
          G_PARAM_READWRITE));
}
...
acmenc_base_init (ACMEncClass *klass)
{
  ...
  params = (ACMEncParams *)
      g_type_get_qdata (G_OBJECT_CLASS_TYPE (klass), ACMENC_PARAMS_QDATA);
  g_assert (params);
  ...
  details.longname = g_strdup_printf ("ACM Encoder: %s", (shortname && *shortname)?shortname:params->name);
  details.klass = "Codec/Encoder/Audio";
  details.description = g_strdup_printf ("ACM Encoder: %s", (longname && *longname)?longname:params->name);
...
acmenc_open_driver (wchar_t *filename)
{
  ...
  res = acmDriverAdd (&driverid, mod, (LPARAM)func, 0, ACM_DRIVERADDF_FUNCTION);
...
static gboolean acmenc_register_file (GstPlugin *plugin, wchar_t *filename)
{
  ACMEncParams *params;
  ...
  name = g_utf16_to_utf8 (filename, (glong)wcslen(filename) - 4, 
      NULL, NULL, NULL);
  params->name = name;
