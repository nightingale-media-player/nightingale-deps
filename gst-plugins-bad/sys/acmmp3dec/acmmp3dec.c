...
static gboolean acmmp3dec_sink_setcaps (GstPad * pad, GstCaps * caps)
{
...
static GstFlowReturn acmmp3dec_chain (GstPad * pad, GstBuffer * buf)
{
  MMRESULT res;
  ACMMP3Dec *dec = (ACMMP3Dec *)GST_PAD_PARENT (pad);
...
static gboolean acmmp3dec_sink_event (GstPad * pad, GstEvent * event)
{
  ACMMP3Dec *dec = (ACMMP3Dec *)GST_PAD_PARENT (pad);
...
{
  dec->sinkpad = gst_pad_new_from_static_template (
          &acmmp3dec_sink_template, "sink");
  ...
  dec->srcpad = gst_pad_new_from_static_template (
          &acmmp3dec_src_template, "src");
